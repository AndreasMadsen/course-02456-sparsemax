\section{Results}
Next the sparsemax transformation is evaluated on a series of benchmark datasets using four different implementations. The model is equivalent to a multivariate logistic regression, but where the activation function at the output layer is sparsemax rather than softmax. \todo{Perhaps we should just write it in the theory section.} For comparison we report the softmax results as well. The four different implementations plus the reference softmax implementation are refered to as:
\begin{itemize}
