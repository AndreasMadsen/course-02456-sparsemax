\section{Results}
Next the sparsemax transformation is evaluated on a series of benchmark datasets using four different implementations. The model is equivalent to a multivariate logistic regression, but where the activation function at the output layer is sparsemax rather than softmax. \todo{Perhaps we should just write it in the theory section.} For comparison we report the softmax results as well. The four different implementations are refered to as:
\begin{itemize}
...
\end{itemize}
For the sake of performance comparison all five implementations are used. For the performance\todo{performance is a bit ambiguous} only the TensorFlow C++ version of Sparsemax and Softmax are used, as the results are the same for all Sparsemax implementations.
...
Where $\mathbf{p}, \mathbf{q}$ are the predicted and target distributions, respectively and $\mathbf{KL}$ is the Kullbach-Leibler distance. For all the classifiers a L2 regualizer is used, the regualization constants used can be seen in Table \ref{tab:hyperparameters}. The hyperparemeter where tuned using a stratified 5-split cross-validation (TODO ?) \todo{only stratified for some datasets}, where the resulting experiments are shown in Figure \ref{fig:hyperparameters}. The models where optimized using the Adam algorithm with default parameters \todo{learning rate was only for numpy implementation} a learning rate of 0.001 and $\beta_1=0.9, \, \beta_2 = 0.999, \, \epsilon = 1\mathrm{e}{-8}$ 
\begin{table}[H]
\centering
...
The resulting performance for the five datasets is showen in Table \ref{tab:results}. The Softmax and Sparsemax classifier are very much alike in terms of perfomance and the Sparsemax classifier only has significant better performance on the Iris dataset.
\begin{table}[H]
\centering
...
For the large dataset, MNIST, the GPU implementation siginficantly outperforms all other Sparsemax implementations as expected. The Numpy implementation runs significantly slower than the three TensorFlow implementations, also as expected. For the Iris dataset, which is considerably smaller, the Numpy and TensorFlow CPU version outperforms both the GPU and Tensorflow Numpy version. Both The TensorFlow Numpy version and the GPU version encours a large overhead, which, as expected, slows the implementation. Compared to the Softmax (TODO on GPU?) the GPU version of TensorFlow is significantly slower, but not by much.

\todo[inline]{missing some information about the number of epochs.}
