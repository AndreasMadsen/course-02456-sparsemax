...
The encoder-decoder model defined in \eqref{eq:attention-model} has been tested on a synthetic dataset. The core task for the model for this data is a translation task. The input sequence is a sequence of numbers spelled by regular characters e.g. "three seven four". This sequence must be translated to the actual numbers e.g. "3 7 4". We randomly generate such sequences for testing and restrict the task to predicting numbers in the range 0-9 and a end of sequence tag denoted by \#. As we suspect that sparse attention will be relatively better for longer sequences, we vary the sequence lengths from five numbers to 20 numbers. 

\subsubsection{Training procedure}
Weights in the model have all been initialized using a truncated random normal with a standard deviation of 0.1 except for bias terms which have been initialized using a constant initializer of 1. 
...
The results of applying sparse attention compared to soft attention can be seen in the attention distributions in figures \ref{fig:sparsemax} and \ref{fig:softmax} together with the validation accuracy.
